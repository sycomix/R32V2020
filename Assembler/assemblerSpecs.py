...
class RegisterParsingAndValidationSpecs(unittest.TestCase):
  def test_rNs_are_valid_registers_from_0_to_15(self):
    for i in xrange(0, 16):
      self.assertTrue(assembler.isValidRegister(f'r{str(i)}'))
  ...
  def test_rNs_just_parse_as_N(self):
    for i in xrange(0, 16):
      self.assertEqual(assembler.parseRegister(f'r{str(i)}'), i)
  ...
  def test_GP0_through_GP7_are_valid_registers(self):
    for i in xrange(0, 8):
      self.assertTrue(assembler.isValidRegister(f'GP{str(i)}'))

  def test_GPN_is_an_alias_for_rN_plus_8(self):
    for i in xrange(0, 8):
      self.assertEqual(assembler.parseRegister(f'GP{str(i)}'), i+8)
...
class DataHexFormatSpecs(unittest.TestCase):
  ...
  def test_full_hello_world_works(self):
    ...
    lines = [
        assembler.formatDataHex(i, result) for i, result in enumerate(results)
    ]
